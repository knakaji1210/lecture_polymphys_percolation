...
'''
How to use
% python3 curveFit_critical_nspc_v2.py args[1] args[2] args[3] args[4]  args[5]
args[1] ./data/h_list_100x100_pc0.6.txt
args[2] ./data/x_list_100x100_pc0.6.txt
args[3] ./data/y_mean_list_100x100_pc0.6.txt
args[4] ./data/y_std_list_100x100_pc0.6.txt 
args[5] ./data/bw_list_100x100_pc0.6.txt 
'''
...
if __name__ == '__main__':
    ...
    ax1 = fig.add_subplot(121, title="Distribution of $s$-clusters at $p$ = $p_{{c}}$", xlabel='Cluster size, $s$', ylabel='Number of s-cluster, $n_{{s}}$($p_{{c}}$)')
    ax1.set_xscale("log")
    ...
    ax2 = fig.add_subplot(122, title='Scaling of Number of $s$-cluster at $p$ = $p_{{c}}$', 
...
#    resultText = "n_s (pc) ∝ tau^("+str(round(param[0],3))+")"
    ...
    result_text5 = "$n_{{s}}$($p_{{c}}$) ∝ $s$$^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(param[0], paramErr)
    fig.text(0.75, 0.8, result_text5)

    savefile = "./png/clusterSize_critical_fit_{0}x{1}_pc{2}.png".format(lattice_x,lattice_y, pc)
