...
#    logparam_u, logcov_u = curve_fit(loglogFit, logcropped_setU[0], logcropped_setU[1])
    ...
    ax2 = fig.add_subplot(122, title='Scaling of Mean Cluster Size', 
            xlabel='Log(pc - p)', ylabel='Log(Mean Cluster Size)')
...
#    resultTextL = "S (p < pc) ∝ (pc - p)^("+str(round(paramset[0][1],2))+")"
    resultTextL = "S (p < pc) ∝ (pc - p)^({0})".format(str(round(paramset[0][1],2)))
#    resultTextU = "P (p > pc) ∝ (p - pc)^("+str(round(paramset[1][1],2))+")"
    resultTextU = "P (p > pc) ∝ (p - pc)^({0})".format(str(round(paramset[1][1],2)))
#    resultTextlogL = "S (p < pc) ∝ (pc - p)^("+str(round(logparamset[0][0],2))+")"
    resultTextlogL = "S (p < pc) ∝ (pc - p)^({0})".format(str(round(logparamset[0][0],2)))
#    resultTextlogU = "P (p > pc) ∝ (p - pc)^("+str(round(logparamset[1][0],2))+")"
    resultTextlogU = "P (p > pc) ∝ (p - pc)^({0})".format(str(round(logparamset[1][0],2)))
