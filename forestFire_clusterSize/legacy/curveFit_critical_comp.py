...
if __name__ == '__main__':
    ...
    logcropped_xL = logcropped_setL[0][-3:] #範囲を指定するためにスライスを使う場合
    logcropped_yL = logcropped_setL[1][-3:] #範囲を指定するためにスライスを使う場合
    logcropped_xU = logcropped_setU[0][:3] #範囲を指定するためにスライスを使う場合
    logcropped_yU = logcropped_setU[1][:3] #範囲を指定するためにスライスを使う場合
...
#    logparam_u, logcov_u = curve_fit(loglogFit, logcropped_setU[0], logcropped_setU[1])
    ...
    ax2 = fig.add_subplot(122, title='Mean Cluster Size vs Probability of Occupying Sites', 
            xlabel='Log(Probability of Occupying Sites)', ylabel='Log(Mean Cluster Size)')
    ...
    ax2.plot(logx_arrayL, logy_arrayL)
    ax2.plot(logx_arrayU, logy_arrayU)

    resultTextL = "S (p < pc) ∝ (pc - p)^("+str(round(paramset[0][1],2))+")"
    resultTextU = "S (p > pc) ∝ (p - pc)^("+str(round(paramset[1][1],2))+")"
    resultTextlogL = "S (p < pc) ∝ (pc - p)^("+str(round(logparamset[0][0],2))+")"
    resultTextlogU = "S (p > pc) ∝ (p - pc)^("+str(round(logparamset[1][0],2))+")"
