# Modeling of forest fire

import numpy as np
from math import *
import clusterConst_regularFiring as cc
import matplotlib.pyplot as plt
import animatplot as amp
from matplotlib import animation
import histDraw as hist

fig = plt.figure(figsize=(16.0, 8.0))

lattice_x = 50
lattice_y = 50
range_x = lattice_x/2
range_y = lattice_y/2
end_point_id = lattice_x * lattice_y

p = 0.58

x_black = []
y_black = []
x_green = []
y_green = []
x_red = []
y_red = []

# Building of initial configuration

initialConfig = cc.initialConfig(lattice_x, lattice_y, p)
greenCount = cc.greenCount(end_point_id, initialConfig)

# Setting a firing point

num_steps = 0
#num_repeat = 10
cluster_list = []

while greenCount > 0:
        fireCount = 0
        while fireCount == 0:
                currentConfig = cc.initialFire(end_point_id, initialConfig)
                fireCount = cc.fireCount(end_point_id, currentConfig)
                

# Stepwise fire spreading
        num_occupied_sites = 0
        while fireCount > 0:
                num_steps+=1
                pointsCoordinates = cc.pointsCoordinates(end_point_id, currentConfig)
                x_black.append(pointsCoordinates[0])
                y_black.append(pointsCoordinates[1])
                x_green.append(pointsCoordinates[2])
                y_green.append(pointsCoordinates[3])
                x_red.append(pointsCoordinates[4])
                y_red.append(pointsCoordinates[5])
                fireCount = cc.fireCount(end_point_id, currentConfig)
                greenCount = cc.greenCount(end_point_id, currentConfig)
#                print(greenCount)
                num_occupied_sites += fireCount
                updatedConfig = cc.fireSpread(lattice_x, lattice_y, end_point_id, currentConfig)
                currentConfig = updatedConfig
        cluster_list.append(num_occupied_sites)

print(cluster_list)

total_num_occupied_sites = np.sum(cluster_list)
p_calc = total_num_occupied_sites / end_point_id
num_clusters = len(cluster_list)
print(num_clusters)
num_sites_max = np.max(cluster_list)
num_sites_mean = np.mean(cluster_list)

resultText1 = "p (set) = "+str(round(p, 2))
resultText2 = "p (sim) = "+str(round(p_calc, 2))
resultText3 = "num_cluster = "+str(round(num_clusters, 2))
resultText4 = "num_sites (max) = "+str(round(num_sites_max, 2))
resultText5 = "num_sites (mean) = "+str(round(num_sites_mean, 2))

#  Animation of forest fire

ax1 = fig.add_subplot(121, title='Forest Fire', xlabel='X', ylabel='Y',
        xlim=[-range_x, range_x], ylim=[-range_y , range_y])
ax1.set_xticks(np.linspace(-range_x, range_x, 5))
ax1.set_yticks(np.linspace(-range_y, range_y, 5))
#ax1.grid(b=True, which='major', color='#999999', linestyle='-')

fig.text(0.70, 0.80, resultText1)
fig.text(0.70, 0.75, resultText2)
fig.text(0.70, 0.70, resultText3)
fig.text(0.70, 0.65, resultText4)
fig.text(0.70, 0.60, resultText5)
hist_x, hist_y, histInfo = hist.histDraw(fig, "cluster size", "green", cluster_list)
binWidth = histInfo[2]
print(binWidth)
print(hist_y)

configration_black = amp.blocks.Scatter(x_black, y_black, ax=ax1, s=15, color='black')
configration_green = amp.blocks.Scatter(x_green, y_green, ax=ax1, s=15, color='green')
configration_red = amp.blocks.Scatter(x_red, y_red, ax=ax1, s=15, color='red')

t = np.linspace(0, num_steps-1, num_steps)
timeline = amp.Timeline(t, units='steps', fps=30)

anim = amp.Animation([configration_black, configration_green, configration_red], timeline)
anim.controls()
anim.save_gif("./gif/forest_fire_regular_firing")

plt.show()