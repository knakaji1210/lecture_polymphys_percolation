...
if __name__ == '__main__':
    try:
        ls = int(input('Lattice Size (default=1,000): '))
    except ValueError:
        ls = 1000
    ...
    max_prob_s_file = "./data/x_list_{0}x{1}_p{2}.txt".format(lattice_size,lattice_size,max_prob)
    max_prob_ns_file = "./data/y_mean_list_{0}x{1}_p{2}.txt".format(lattice_size,lattice_size,max_prob)
    ...
    for n in range(num_p):
        try:
            ...
            p = 0.58
        prob = str(p)
        ns_file = "./data/y_mean_list_{0}x{1}_p{2}.txt".format(lattice_size,lattice_size,prob)
