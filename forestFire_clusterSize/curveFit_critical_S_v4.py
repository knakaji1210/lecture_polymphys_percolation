...
args[3] ./data/S_mean_list_100x100_p0.4-0.8.txt
args[4] ./data/S_std_list_100x100_p0.4-0.8.txt
'''
...
#    logy_arrayU = fittedArray(logx_arrayU, logparamset, "u_log")
    ...
    y_max = np.max(y_arrayL) # 250125範囲指定を追加（Sだけに集中するため）

    ax1 = fig.add_subplot(121, title='Mean Cluster Size vs Probability of Occupying Sites', 
            xlabel='$p$', ylabel='$S$', xlim=(p_min, pc), ylim=(-0.2*y_max, 1.1*y_max)) # 250125範囲指定を追加（Sだけに集中するため）
    ...
    resultTextL = "$S$ ($p$ < $p_{{c}}$) ∝ ($p_{{c}}$ - $p$)$^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(paramset[0][1], paramset[1])
    resultTextlogL = "$S$ ($p$ < $p_{{c}}$) ∝ ($p_{{c}}$ - $p$)$^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(logparamset[0][0],logparamset[1])
