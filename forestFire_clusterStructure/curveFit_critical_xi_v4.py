...
'''
How to use
% python3 curveFit_critical_xi_v3.py args[1] args[2] args[3] args[4]
args[1] ./data/h_list_100x100_p0.4-0.8.txt
args[2] ./data/p_list_100x100_p0.4-0.8.txt
args[3] ./data/xi_mean_list_100x100_p0.4-0.8.txt
args[4] ./data/xi_std_list_100x100_p0.4-0.8.txt
'''
...
#    logy_arrayU = fittedArray(logx_arrayU, logparamset, "u_log")
    ...
    y_max = np.max(y_arrayL) # 250126範囲指定を追加（Sだけに集中するため）

    ax1 = fig.add_subplot(121, title='Mean Correlation Length vs Probability of Occupying Sites', 
            xlabel='$p$', ylabel='$ξ$', xlim=(p_min, pc), ylim=(-0.2*y_max, 1.1*y_max)) # 250126範囲指定を追加（Sだけに集中するため）)
    ...
    resultTextL = "$ξ$ ($p$ < $p_{{c}}$) ∝ ($p_{{c}}$ - $p$)$^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(paramset[0][1], paramset[1])
    resultTextlogL = "$ξ$ ($p$ < $p_{{c}}$) ∝ ($p_{{c}}$ - $p$)$^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(logparamset[0][0],logparamset[1])
    fig.text(0.30, 0.17, resultTextL)
    fig.text(0.57, 0.17, resultTextlogL)
