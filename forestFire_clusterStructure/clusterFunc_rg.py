...
def initialFire(end_point_id, currentConfig):
...
# function for initial firing points
    for i in range(end_point_id):
        ...
        if point_status == 0:
            ...
            point_attribute[1] = 2
            int_fired_point_coordinate = point_attribute[2] #cluster structureのために追加
            break
    return currentConfig, int_fired_point_coordinate
        ...
        if point_id % lattice_y == lattice_y - 1: #ポイントが北一列の場合
            ...
            if point_status == 2:
                ...
                if pointW_status == 1:
                    pointW_attribute[1] = -1
                point_attribute[1] = 0
    cluster_coordinates_diff = []
    for j in range(end_point_id):
        point_attribute = currentConfig[j]
        point_status = point_attribute[1]
        if point_status == -1:
            point_attribute[1] = 2
            fired_point_coordinate = point_attribute[2] #cluster structureのために追加
            cluster_coordinates_diff.append(fired_point_coordinate) #cluster structureのために追加
    return currentConfig, cluster_coordinates_diff
...
def greenCount(end_point_id, currentConfig):

# function for survived green
